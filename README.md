...
## Major revision (version 2.0.0)

The revision that accompanies the published article adds two top-level items to the
version 1.0.0 layout:

```text
revision_analysis/              Revision analysis scripts r01 to r17 and their tests
outputs/11_major_revision/
  analysis/                     Reference, scoring, statistical, runtime, resampling
                                and symbolic-audit outputs
  figures/                      Main and supplementary figure files
  config/                       Entropy and runtime protocol configurations
```

Everything under `outputs/01_stacks` to `outputs/10_review_response_computations` is the
version 1.0.0 payload and is retained unchanged so that the earlier record remains
reproducible. Where the two disagree, the version 2.0.0 outputs under
`outputs/11_major_revision/` are the ones reported in the article. See `CHANGELOG.md`
for the corrections that produced those differences.
